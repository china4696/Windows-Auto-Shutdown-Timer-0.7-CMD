...
::
::YAwzoRdxOk+EWAjk
::fBw5plQjdDWDJNFjPsfb3ZKjsIJLw+Ny7jPlCm0CJ7rJ9XEcA7Y7dcKM0eOWL61DuhSkPcdkljIL0YUFDxQ4
...
::Zh4grVQjdDWDJNFjPsfb3ZKjsIJLw+Ny7jPlCm0CJ7rJ9XGVsyi6k3pSLUgI6R+bIq1ijHSoL7IK1pxCCQNdHg==
::YB416Ek+ZG8=
::
::
...
:input_0000
cls
start "Countdown Reminder" mshta "javascript:alert('此程序目前正在測試中可能會有bug ');close();"

color 04
rem 用戶選擇是否要啟動自動關機計時器
echo === 自動關機計時器0.8 beta===
echo **腳本目的**
...
if %result% equ 0 (
    echo 指令執行成功！
    start "Command Result" mshta "javascript:alert('指令執行成功！您的電腦已完成操作: %command%');close();"
) else (
    echo 指令執行失敗！錯誤碼: %result%
    start "Command Result" mshta "javascript:alert('指令執行失敗！錯誤碼: %result%');close();"
)
